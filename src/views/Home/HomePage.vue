...
const typeBooks = [{id: 1, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护!'},
  {id: 2, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'},
  {id: 3, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'},
  {id: 4, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'},
  {id: 5, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'},
  {id: 1, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护!'},
  {id: 2, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'},
  {id: 3, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'},
  {id: 4, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'},
  {id: 5, name: '被反派哥哥读心之后', author: '月下蝶影', shortProfile: '这一次，哥哥们由我来守护'}]
...
const collectionRank = ref(
  [numberBooks,numberBooks,numberBooks,numberBooks,numberBooks,numberBooks,numberBooks,numberBooks,numberBooks,numberBooks]
)

const endRank = ref(
  [noNumberBooks,noNumberBooks,noNumberBooks,noNumberBooks,noNumberBooks,noNumberBooks,noNumberBooks,noNumberBooks,noNumberBooks,noNumberBooks]
)
  ...
  <div>
    ...
    <nav class="nav">
      <div class="w">
        ...
        <div class="navitems">
          <ul>
            <li><router-link :to="{ name: 'All' }">全部作品</router-link></li>
    ...
    <div class="w">
      <div class="carousel">
        <el-carousel height="270px" ref="carouselRef" @change="handleSlideChange">
          <el-carousel-item v-for="item in carouselItems" :key="item" :name="item.name">
            <el-image :src="item.img" alt="carousel image" class="carousel-image" />
          </el-carousel-item>
        </el-carousel>
      ...
      <div class="announcements">
        ...
        <ul>
          <li v-for="item in announcementsItems.slice(0, 8)" :key="item.id">
            <router-link :to="{ name: 'BookDetail', params: { id: item.id } }">{{ item.content }}</router-link>
          </li>
        </ul>
      </div>
      <div class="newBookRecommend">
        ...
        <ul>
          <li v-for="item in recommendNewBooks.slice(0, 11)" :key="item.id">
            <span>[{{ item.type }}]</span>
            <router-link :to="{ name: 'BookDetail', params: { id: item.id } }">{{item.name}}</router-link>
          </li>
        </ul>
      </div>
    </div>
    <div class="rank w">
      <!--      月票榜(这个月得到的月票排行)-->
      <div class="pass">
        <RankList title="月票榜" type="0" :books="passRank" :img="defaultCover"></RankList>
      </div>
      <!--      畅销榜(这个月得到的订阅点数排行)-->
      <div class="selling">
        <RankList title="畅销榜" type="1" :books="sellingRank" :img="defaultCover"></RankList>
      </div>
      <!--      月推荐榜(这个月得到的推荐票总数)-->
      <div class="recommend">
        <RankList title="月推荐榜" type="2" :books="passRank" :img="defaultCover"></RankList>
      </div>
      <!--      新书榜(这个月新书的销量排行)-->
      <div class="newBook">
        <RankList title="新书榜" type="1" :books="sellingRank" :img="defaultCover"></RankList>
      </div>
    </div>
    <div class="list w">
      <div class="type" v-for="(typeList,index) in typeLists" :key="index">
        <TypeList :books="typeList.books" :title="typeList.type"></TypeList>
      </div>
    </div>
    <div class="end">
      <RankList title="完结榜" type="1" :books="endRank" :img="defaultCover"></RankList>
    </div>
